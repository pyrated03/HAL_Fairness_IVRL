...
class GaussianHSICClassification(pl.LightningModule):
    def __init__(self, opts, dataloader):
        ...
        self.model_adv = getattr(models, opts.model_adv_type)(
            **opts.model_options_adv)
        self.rbfsigma = getattr(models, opts.gaussian_sigma)()

        self.criterion = {}
        self.criterion['tgt_loss'] = getattr(
            losses, opts.loss_type)(**opts.loss_options)
        self.criterion['test_tgt_loss'] = getattr(
            losses, opts.loss_type)(**opts.loss_options)
        self.criterion['val_tgt_loss'] = getattr(
            losses, opts.loss_type)(**opts.loss_options)

        self.loss_tgt_trn = getattr(metrics, opts.evaluation_type)(
            **opts.evaluation_options)
        self.loss_tgt_val = getattr(metrics, opts.evaluation_type)(
            **opts.evaluation_options)
        self.loss_tgt_tst = getattr(metrics, opts.evaluation_type)(
            **opts.evaluation_options)
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log('train_hsic_loss', hsic, on_step=False,
        ...
        self.log('train_loss', loss, on_step=False,
                 on_epoch=True, prog_bar=True)
        self.log('train_tgt_loss', tgt_loss, on_step=False,
                 on_epoch=True, prog_bar=True)
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.hsic = torch.trace(torch.mm(K_zm, K_sm)) / torch.sqrt(
            torch.trace(torch.mm(K_sm, K_sm)) * torch.trace(torch.mm(K_zm, K_zm)))
        # hsic = torch.trace(torch.mm(K_zm, K_sm)) / (n ** 2)
        ...
        if self.data_flag is not None:
            if self.data_flag:
                ...
                self.log('test_test_tgt_loss', tgt_loss,
                         on_step=False, on_epoch=True)
                self.log('test_test_hsic_zs', self.hsic,
                ...
                self.log('test_train_tgt_loss', tgt_loss,
                         on_step=False, on_epoch=True)
                self.log('test_train_hsic_zs', self.hsic,
                         on_step=False, on_epoch=True, prog_bar=True)
        ...
        if self.data_flag is not None:
            if self.data_flag:
                ...
                metrics_dict['test_acc'] = float(
                    self.acc_tst.compute().cpu().numpy().astype(np.float128))

                metrics_dict['test_test_hsic_zs'] = float(
                    self.hsic.cpu().numpy().astype(np.float128))
                ...
                metrics_dict['test_train_hsic_zs'] = float(
                    self.hsic.cpu().numpy().astype(np.float128))
